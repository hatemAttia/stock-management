...
app.get("/api/stats/top-products", (req, res) => {
  ...
  sales.forEach((r) => {
    if (!map[r.product_id])
      map[r.product_id] = { product_id: r.product_id, units_sold: 0, revenue: 0 };
    ...
    Object.values(map)
      .map((row) => {
        ...
        const cost = purchases
          .filter((x) => x.product_id === row.product_id)
          .reduce((s, x) => s + x.total_cost, 0);
        return {
          name: p.name || "?",
...
// Sales breakdown per product per day (for tree reports)
app.get("/api/stats/sales-by-product", (req, res) => {
  ...
  const map = {};
  list.forEach((r) => {
    const key = `${r.date}__${r.product_id}`;
    if (!map[key]) {
      const p = products.find((x) => x.id === r.product_id) || {};
      map[key] = { date: r.date, product: p.name || "?", unit: p.unit || "", qty: 0, revenue: 0 };
    }
    map[key].qty     += r.quantity;
    map[key].revenue += r.total_price;
  });
  res.json(Object.values(map).sort((a, b) => b.date.localeCompare(a.date) || a.product.localeCompare(b.product)));
});
