...
function renderProductTable(data) {
  ...
  makeSortableTable("productTable", data, [
    ...
    { key: "_del",       label: "",        nosort: true,
      render: r => `<button class="btn btn-danger btn-sm" onclick="deleteProduct(${r.id})">🗑</button>` },
  ]);
}
...
function renderPurchaseTable(data) {
  ...
  makeSortableTable("purchaseTable", data, [
    { key: "date",         label: "Date" },
    { key: "product_name", label: "Product", render: r => `<strong>${r.product_name}</strong>` },
    ...
    { key: "_del",         label: "", nosort: true,
      render: r => `<button class="btn btn-danger btn-sm" onclick="deletePurchase(${r.id})">🗑</button>` },
  ], footer);
}
...
function renderStockTable(data) {
  ...
  makeSortableTable("stockTable", data, [
    ...
    { key: "stock_remaining",    label: "Remaining",  render: r => `<strong>${r.stock_remaining}</strong>` },
    { key: "total_cost_purchased", label: "Total Cost", render: r => fmt(r.total_cost_purchased) },
    { key: "stock_remaining",    label: "Status", nosort: true, render: r => {
        const cls  = r.stock_remaining <= 0 ? "badge-red" : r.stock_remaining < 5 ? "badge-yellow" : "badge-green";
        const text = r.stock_remaining <= 0 ? "Out of stock" : r.stock_remaining < 5 ? "Low stock" : "In stock";
...
function renderReportTable(daily, costMap) {
  ...
  makeSortableTable("reportTable", enriched, [
    { key: "date",         label: "Date",             render: r => `<strong>${r.date}</strong>` },
    { key: "transactions", label: "Transactions" },
    { key: "units_sold",   label: "Units Sold" },
    { key: "revenue",      label: "Revenue",   render: r => `<span style="color:var(--success)">${fmt(r.revenue)}</span>` },
    { key: "cost",         label: "Cost",      render: r => `<span style="color:var(--danger)">${fmt(r.cost)}</span>` },
    { key: "profit",       label: "Profit",    render: r => `<span style="color:${r.profit >= 0 ? "var(--success)" : "var(--danger)"};font-weight:700">${fmt(r.profit)}</span>` },
  ], footer);
}
