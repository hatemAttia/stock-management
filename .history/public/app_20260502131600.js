...
function renderPurchaseTable(data) {
  ...
  const groups = {};
  data.forEach((r) => {
    if (!groups[r.product_name]) groups[r.product_name] = { unit: r.unit, rows: [] };
    groups[r.product_name].rows.push(r);
  });

  const grandTotal = data.reduce((s, r) => s + r.total_cost, 0);
  let rows = "";
  let gi = 0;

  Object.entries(groups)
    .sort(([a], [b]) => a.localeCompare(b))
    .forEach(([productName, g]) => {
      const groupTotal = g.rows.reduce((s, r) => s + r.total_cost, 0);
      const groupQty   = g.rows.reduce((s, r) => s + r.quantity, 0);
      const gid = `pu-group-${gi++}`;
      rows += `
        <tr class="tree-parent" onclick="toggleTree('${gid}')">
          <td colspan="7">
            <span class="tree-toggle">▶</span>
            <span class="tree-label">📦 ${productName}</span>
            <span class="tree-summary">
              <span class="tree-sum">${g.rows.length} entries</span>
              <span class="tree-sum yellow">${groupQty} ${g.unit}</span>
              <span class="tree-sum red">${fmt(groupTotal)}</span>
            </span>
          </td>
        </tr>`;
      g.rows.forEach((r) => {
        rows += `
          <tr class="tree-child" data-group="${gid}">
            <td>${r.date}</td>
            <td>${r.quantity} ${r.unit}</td>
            <td>${fmt(r.unit_cost)}</td>
            <td><span class="badge badge-red">${fmt(r.total_cost)}</span></td>
            <td>${r.supplier || "—"}</td>
            <td colspan="2"><button class="btn btn-danger btn-sm" onclick="deletePurchase(${r.id})">🗑</button></td>
          </tr>`;
      });
    });
  ...
  el.innerHTML = `<table>
    <thead><tr>
      <th>Date</th><th>Qty</th><th>Unit Cost</th>
      <th>Total Cost</th><th>Supplier</th><th colspan="2"></th>
    </tr></thead>
    <tbody>${rows}</tbody>
    <tfoot><tr style="background:var(--surface2)">
      <td colspan="3" style="padding:10px 16px;font-weight:700">Grand Total</td>
      <td style="padding:10px 16px;font-weight:700;color:var(--danger)">${fmt(grandTotal)}</td>
      <td colspan="3"></td>
    </tr></tfoot>
  </table>`;
}
...
function renderSaleTable(data) {
  ...
  const grandTotal = data.reduce((s, r) => s + r.total_price, 0);
  let rows = "";
  let gi = 0;
  ...
  Object.entries(groups)
    .sort(([a], [b]) => a.localeCompare(b))
    .forEach(([productName, g]) => {
      const groupTotal = g.rows.reduce((s, r) => s + r.total_price, 0);
      const groupQty   = g.rows.reduce((s, r) => s + r.quantity, 0);
      const gid = `sa-group-${gi++}`;
      rows += `
        <tr class="tree-parent" onclick="toggleTree('${gid}')">
          <td colspan="7">
            <span class="tree-toggle">▶</span>
            <span class="tree-label">🏷️ ${productName}</span>
            <span class="tree-summary">
              <span class="tree-sum">${g.rows.length} entries</span>
              <span class="tree-sum yellow">${groupQty} ${g.unit}</span>
              <span class="tree-sum green">${fmt(groupTotal)}</span>
            </span>
          </td>
        </tr>`;
      g.rows.forEach((r) => {
        rows += `
          <tr class="tree-child" data-group="${gid}">
            ...
            <td>${r.client || "—"}</td>
            <td colspan="2"><button class="btn btn-danger btn-sm" onclick="deleteSale(${r.id})">🗑</button></td>
          </tr>`;
      });
    });
  ...
  el.innerHTML = `<table>
    <thead><tr>
      <th>Date</th><th>Qty</th><th>Unit Price</th>
      <th>Revenue</th><th>Client</th><th colspan="2"></th>
    </tr></thead>
    <tbody>${rows}</tbody>
    <tfoot><tr style="background:var(--surface2)">
      <td colspan="3" style="padding:10px 16px;font-weight:700">Grand Total</td>
      <td style="padding:10px 16px;font-weight:700;color:var(--success)">${fmt(grandTotal)}</td>
      <td colspan="3"></td>
    </tr></tfoot>
  </table>`;
}
...
async function loadReports() {
  ...
  const [daily, purchases, byProduct] = await Promise.all([
    api(`/stats/daily-sales?from=${fromEl.value}&to=${toEl.value}`),
    api(`/purchases?from=${fromEl.value}&to=${toEl.value}`),
    api(`/stats/sales-by-product?from=${fromEl.value}&to=${toEl.value}`),
  ]);
  ...
  // Group byProduct rows by date
  const productsByDate = {};
  byProduct.forEach((r) => {
    if (!productsByDate[r.date]) productsByDate[r.date] = [];
    productsByDate[r.date].push(r);
  });

  renderReportTable(daily, costMap, productsByDate);
  buildReportChart(daily, costMap);
}

function renderReportTable(daily, costMap, productsByDate) {
  ...
  daily.forEach((r) => {
    ...
    const gid    = `rp-group-${gi++}`;
    const children = productsByDate[r.date] || [];

    rows += `
      <tr class="tree-parent" onclick="toggleTree('${gid}')">
        <td>
          <span class="tree-toggle">▶</span>
          <span class="tree-label">${r.date}</span>
        </td>
    ...
    children.forEach((c) => {
      rows += `
        <tr class="tree-child" data-group="${gid}">
          ...
          <td>—</td>
          <td>${c.qty} ${c.unit}</td>
          <td style="color:var(--success)">${fmt(c.revenue)}</td>
          <td>—</td>
          <td>—</td>
        </tr>`;
    });
...
function toggleTree(gid) {
  const parent = document.querySelector(`tr.tree-parent[onclick="toggleTree('${gid}')"]`);
  const children = document.querySelectorAll(`tr.tree-child[data-group="${gid}"]`);
  const isOpen = parent.classList.toggle("open");
  children.forEach((c) => c.classList.toggle("visible", isOpen));
}
