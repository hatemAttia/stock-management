...
app.get("/api/stock", async (req, res) => {
  ...
  const stock = products.map((p) => {
    ...
    const totalPurchased = purchases.filter(x => String(x.product_id) === id).reduce((s, x) => s + x.quantity, 0);
    const totalSold      = sales.filter(x => String(x.product_id) === id).reduce((s, x) => s + x.quantity, 0);
    const totalCost      = purchases.filter(x => String(x.product_id) === id).reduce((s, x) => s + x.total_cost, 0);
    return { id, name: p.name, unit: p.unit, total_purchased: totalPurchased, total_sold: totalSold, stock_remaining: totalPurchased - totalSold, total_cost_purchased: totalCost };
  });
  res.json(stock);
});
...
app.get("/api/purchases", async (req, res) => {
  ...
  const list = await Purchase.find(filter).populate("product_id", "name unit").sort({ date: -1 }).lean();
  res.json(list.map(r => ({ ...r, id: r._id, product_name: r.product_id?.name || "?", unit: r.product_id?.unit || "" })));
});
...
app.post("/api/purchases", async (req, res) => {
  ...
  const r = await Purchase.create({ product_id, quantity: parseFloat(quantity), unit_cost: parseFloat(unit_cost), total_cost, supplier: supplier || "", date });
...
app.get("/api/sales", async (req, res) => {
  ...
  if (from || to) { filter.date = {}; if (from) filter.date.$gte = from; if (to) filter.date.$lte = to; }
  if (product_id) filter.product_id = product_id;
  const list = await Sale.find(filter).populate("product_id", "name unit").sort({ date: -1 }).lean();
  res.json(list.map(r => ({ ...r, id: r._id, product_name: r.product_id?.name || "?", unit: r.product_id?.unit || "" })));
});
...
app.post("/api/sales", async (req, res) => {
  ...
  const remaining = purchases.reduce((s, x) => s + x.quantity, 0) - sales.reduce((s, x) => s + x.quantity, 0);
  if (remaining < qty)
    return res.status(400).json({ error: `Insufficient stock. Available: ${remaining}` });

  const total_price = qty * parseFloat(unit_price);
  const r = await Sale.create({ product_id, quantity: qty, unit_price: parseFloat(unit_price), total_price, client: client || "", date });
  res.json({ ...r.toObject(), id: r._id });
});
...
app.get("/api/stats/top-products", async (req, res) => {
  ...
  sales.forEach(r => {
    ...
    if (!map[key]) map[key] = { name: r.product_id?.name || "?", units_sold: 0, revenue: 0, pid: key };
    map[key].units_sold += r.quantity;
    map[key].revenue    += r.total_price;
  });
  const result = Object.values(map).map(row => {
    const cost = purchases.filter(x => String(x.product_id) === row.pid).reduce((s, x) => s + x.total_cost, 0);
...
app.get("/api/stats/dashboard", async (req, res) => {
  ...
  const lowStock = products.map(p => {
    const id = String(p._id);
    const tp = purchases.filter(x => String(x.product_id) === id).reduce((s, x) => s + x.quantity, 0);
    const ts = sales.filter(x => String(x.product_id) === id).reduce((s, x) => s + x.quantity, 0);
