...
async function main() {
  ...
  console.log(`📂  Found: ${products.length} products, ${purchases.length} purchases, ${sales.length} sales\n`);
  ...
  for (const p of products) {
    const res = await request("POST", `${BASE_URL}/products`, { name: p.name, unit: p.unit || "unit" });
    if (res.status === 200 || res.status === 201) {
      ...
      const existing = listRes.body.find(x => x.name.toLowerCase().trim() === p.name.toLowerCase().trim());
      if (existing) {
